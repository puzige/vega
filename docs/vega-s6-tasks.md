...
- 所有 Git 命令共享 exact global prefix：由 R0a fixed resolver 选定并进程内固定的 canonical Git executable（macOS arm64 依次为 `/opt/homebrew/opt/git/bin/git`、`/usr/bin/git`；Intel 依次为 `/usr/local/opt/git/bin/git`、`/usr/bin/git`；要求 Git 2.40+）后接 `--no-pager -c core.fsmonitor=false -c color.ui=false -c maintenance.auto=false -c maintenance.autoDetach=false -c gc.auto=0`。trusted stage/commit/switch 再追加 `-c core.hooksPath=/dev/null`；不允许其他用户/模型 option。
