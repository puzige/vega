...
- 任何可能触发 Git content conversion 的 status/diff/stage/switch 前，tracked relevant raw path set 必须由 fixed `ls-files -z --cached --deduplicate` 取得，并以 bounded stdin 交 exact `check-attr -z --stdin --all`（target tree exact顺序为 `check-attr --source=<captured_oid> -z --stdin --all`）；输出只要显式出现 attribute name=`filter`，无论 value 为何均保守拒绝。`--deduplicate`防止unmerged stage1/2/3重复路径误砖死，private parser仍拒绝其输出中的duplicate。untracked direct-read不经过 Git conversion，不纳入此 tracked set。preflight 前后 bytes-exact 重查；不得以 `unspecified`/`unset` 特判放行显式 filter，也不得解析或执行 filter config。
...
- selected `WorkspaceFileId` 展开 literal raw set：rename=old+new，delete=old，add/untracked=new，modify=current；bytes-first dedupe。任何 selected changed `.gitattributes` 直接拒绝。tracked expanded set 经 exact `git check-attr -z --stdin --all`；输出只要显式出现 attribute name=`filter`，无论 value 为何均 fail closed。
