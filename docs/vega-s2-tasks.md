...
> v0.2 变更（2026-08-29，人类决策）：T12 增补 **文件布局重构**（tech-spec §6 Zed 式混合：配置 `~/.config/vega/`、数据 macOS `~/Library/Application Support/ai.vega/`），随 T12 实施。
...
- **产出**：
  ...
  - **文件布局重构（v0.2 增补，人类决策 Zed 式混合）**：vega_store 新增零依赖 `paths` 模块——配置根 `${XDG_CONFIG_HOME:-~/.config}/vega`；数据根按平台 `cfg!(target_os)` 门控：macOS `$HOME/Library/Application Support/ai.vega`，其余平台 `${XDG_DATA_HOME:-~/.local/share}/vega`；config.toml 与 vega.db 迁至新路径（tech-spec §2/§6 已修订）；不做 `~/.vega` 自动迁移；受影响测试改为显式路径注入（config 已有 load_from/save_to 模式）
