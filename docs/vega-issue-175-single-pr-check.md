...
> 2026-09-25 [Issue #188](vega-issue-188-parallel-ci-checks.md) supersedes the single-job topology below: independent macOS Clippy (fmt then Clippy) and Nextest jobs run in parallel, each restoring the shared master cache without saving from PRs. A lightweight aggregate retains the required check name `check (fmt, clippy, test)`, runs with `always()`, and fails unless both dependency results are `success`, including failure, cancellation, and skipped results. [Issue #183](vega-issue-183-nextest-ci.md) still governs the runner: pinned `nextest@0.9.146`, only `cargo nextest run --workspace`, default concurrency/profile, no shards, archives, extra build, or Cargo test command. CI covers unit and integration tests, not doctests. Master packaging is unchanged. The original scope and verification record below describe the historical #175 implementation.
