...
本文件把用户提供的桌面端设计 token 参考整理为 Vega 自己的原生设计守则。它吸收通用的比例、语义 token、状态与可访问性原则，但不复制 Web/Tailwind 实现、VS Code 兼容变量、CSS `data-*` API、第三方私有命名或资产。
...
[竞品设计参考](vega-design-reference.md)仅是历史研究材料，不是实现依据。禁止从解包应用复制 CSS、JavaScript、内部 token 名、图标或其他资产。
