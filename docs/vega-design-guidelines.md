...
- Sidebar 搜索入口使用顶部工具栏尾部的共享放大镜图标按钮，带“搜索 (⌘K)”可访问名称与提示，点击或键盘激活打开现有搜索面板；保留 Command-K。移除独立搜索行及其占位，组织列表自然上移。此规则取代 R34 的双行快捷键对齐规则，精确规格见 [R39 Sidebar search icon](vega-r39-search-icon.md)。
