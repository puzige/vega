...
用户确认（2026-09-14）：**选择档位后，模型弹窗保持打开。**

理由：档位选择会触发一次持久化往返（`ThinkingTierSelected` → `ComposerDefaultsRequested` → 写 `reasoning.toml`）。若 ack 到达时关闭弹窗，会在用户拖动过程中把控件抽走。保持打开让用户能连续调整。

**仅切换模型时关闭弹窗**（既有行为）。

**测试要求**：断言选中档位后弹窗仍处于打开状态。
