...
- 云端流水线拆成两条独立 workflow，共享同一个 cargo 缓存 key（`shared-key: vega`，2026-09-22 用户裁决）：
  - `.github/workflows/pr-check.yml`：`pull_request`（base `master`）跑 `cargo fmt --all -- --check` → `cargo clippy --workspace --all-targets -- -D warnings` → `cargo nextest run --workspace`（`.config/nextest.toml`，`retries = 0`）→ `cargo test --workspace --doc`（nextest 不跑 doc-tests，单独保留）；只读缓存（`save-if: false`）。
