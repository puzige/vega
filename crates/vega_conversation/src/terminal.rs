...
mod tests {
    use super::*;
    use std::{fs, process::Command as ProcessCommand, time::Instant};

    fn contents(snapshot: &TerminalSnapshot) -> String {
        snapshot
            .cells
            .iter()
            .map(|row| {
    ...
    fn wait_for(
        ...
        loop {
            if let Some(snapshot) = session.snapshot(None)
                && predicate(&snapshot)
            {
                return snapshot;
            }
            assert!(
                ...
                "bounded terminal condition failed"
            );
            std::thread::sleep(Duration::from_millis(15));
        }
    }
    #[test]
    fn owned_login_pty_persistent_interrupt_resize_exit_and_reap() {
        const MARKER: &str = "VEGA_R11_OWNED_TERMINAL_TEST";
        if let Some(root) = std::env::var_os(MARKER) {
            let root = PathBuf::from(root);
            let mut session = TerminalSession::start(&root).unwrap();
            wait_for(&session, |s| s.status == TerminalStatus::Running);
            session
                .input(b"printf '%s' \"$$\" > shell-pid; printf 'READY_%s\\n' PTY\r")
                .unwrap();
            wait_for(&session, |s| contents(s).contains("READY_PTY"));
            let pid: i32 = fs::read_to_string(root.join("shell-pid"))
                .unwrap()
                .parse()
                .unwrap();
            session.input(b"export VEGA_PTY_STATE=retained; mkdir nested; cd nested; pwd > ../actual-cwd\r").unwrap();
            session
                .input(b"printf 'STATE_%s\\n' \"$VEGA_PTY_STATE\"\r")
                .unwrap();
            wait_for(&session, |s| contents(s).contains("STATE_retained"));
            assert_eq!(
                fs::read_to_string(root.join("actual-cwd")).unwrap().trim(),
                root.join("nested")
                    .canonicalize()
                    .unwrap()
                    .to_str()
            ...
            session
                .input(b"stty size > ../actual-size; printf 'SIZE_%s\\n' DONE\r")
                .unwrap();
            wait_for(&session, |s| contents(s).contains("SIZE_DONE"));
            assert_eq!(
                fs::read_to_string(root.join("actual-size")).unwrap().trim(),
                "31 97"
            );
            session
                .input(b"sleep 30; printf 'SLEEP_%s\\n' FINISHED\r")
                .unwrap();
            std::thread::sleep(Duration::from_millis(100));
            session.input(&[3]).unwrap();
            session
                .input(b"printf 'INTERRUPT_%s\\n' RECOVERED\r")
                .unwrap();
            wait_for(&session, |s| contents(s).contains("INTERRUPT_RECOVERED"));
            session
                .input(b"printf '\\033[31mANSI_%s\\033[0m\\n' RED\r")
                .unwrap();
            let screen = wait_for(&session, |s| contents(s).contains("ANSI_RED"));
            assert!(
                screen
                    .cells
                    .iter()
            ...
            wait_for(&session, |s| s.status == TerminalStatus::Exited(7));
            session.close();
            // SAFETY: read-only existence check for the PID emitted by this owned shell.
            assert_eq!(unsafe { libc::kill(pid, 0) }, -1, "shell was reaped");
            let mut restarted = TerminalSession::start(&root).unwrap();
            wait_for(&restarted, |s| s.status == TerminalStatus::Running);
            restarted
                .input(b"printf '%s' \"$$\" > restart-pid; sleep 30\r")
                .unwrap();
            let deadline = Instant::now() + Duration::from_secs(8);
            while !root.join("restart-pid").exists() {
                assert!(Instant::now() < deadline);
                std::thread::sleep(Duration::from_millis(15));
            }
            let pid: i32 = fs::read_to_string(root.join("restart-pid"))
                .unwrap()
                .parse()
                .unwrap();
            restarted.close();
            ...
            assert_eq!(
                ...
                -1,
                "explicit close kills and reaps"
            );
            return;
        }
        // Execute the exact production path under an owned shell home/ZDOTDIR without mutating
        // the test runner's environment or reading the user's shell startup files.
        let root = tempfile::tempdir().unwrap();
        let output = ProcessCommand::new(std::env::current_exe().unwrap())
            .args([
                "--exact",
                "terminal::tests::owned_login_pty_persistent_interrupt_resize_exit_and_reap",
                "--nocapture",
            ])
        ...
        assert!(
            ...
            "owned terminal subprocess failed: {}",
