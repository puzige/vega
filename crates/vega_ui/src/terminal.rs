...
mod tests {
    ...
    fn wait_ui(
        ...
        loop {
            ...
            assert!(
                Instant::now() < until,
                "terminal UI timeout during {phase}: {:?}",
                view.read_with(cx, |view, _| (
                    view.snapshot.as_ref().map(|s| (s.status, s.generation)),
                    view.size,
                    view.input_error,
                ))
            );
    ...
    fn prompt_epoch(root: &std::path::Path) -> Option<u64> {
        std::fs::read_to_string(root.join("ui-prompt-count"))
            .ok()?
            .trim()
            .parse()
            .ok()
    ...
    fn prompt_after(view: &TerminalView, root: &std::path::Path, previous: u64) -> bool {
        let Some(epoch) = prompt_epoch(root).filter(|epoch| *epoch > previous) else {
            return false;
        };
        view.snapshot.as_ref().is_some_and(|snapshot| {
            snapshot
                ...
                .collect::<String>()
                .contains(&format!("VEGA_UI_READY_{epoch}>"))
        })
    }
    ...
    async fn production_terminal_input_handler_and_keys_reach_real_pty(cx: &mut TestAppContext) {
        ...
        let Some(root) = std::env::var_os(MARKER) else {
            ...
            // The prompt sequence is generated by the real shell, never typed
            // through the input handler. Unique epochs cannot match old rows,
            // echoed commands, or a screen that has scrolled.
            std::fs::write(
                root.path().join("ui-prompt.sh"),
                "n=0\nif test -f ui-prompt-count; then read -r n < ui-prompt-count; fi\nn=$((n+1))\nprintf '%s\\n' \"$n\" > ui-prompt-count\nprintf 'VEGA_UI_READY_%s> ' \"$n\"\n",
            )
            .unwrap();
            let prompt = "PS1='$(/bin/sh ./ui-prompt.sh)'\nexport PS1\n";
            std::fs::write(root.path().join(".profile"), prompt).unwrap();
            std::fs::write(
                root.path().join(".zshrc"),
                format!("setopt PROMPT_SUBST\n{prompt}"),
            )
            .unwrap();
            std::fs::write(
                root.path().join("ui-foreground.sh"),
                "trap 'printf interrupted > ui-interrupted; exit 130' INT\nprintf ready > ui-foreground-ready\nsleep 30\nprintf completed > ui-uninterrupted\n",
            )
            .unwrap();
        ...
        window
            ...
            .unwrap();
        wait_ui(&view, cx, "initial shell prompt", |view| {
            view.snapshot
                .as_ref()
                .is_some_and(|s| s.status == TerminalStatus::Running)
                && prompt_after(view, &root, 0)
        });
        let initial_size = view.read_with(cx, |view, _| view.size);
        {
            ...
            // R47 §2.3: while Running the status toolbar row (status label,
            // copy and restart) is not rendered at all.
            assert!(
        ...
        window
            ...
            .unwrap();
        wait_ui(&view, cx, "resized canvas", |view| {
        ...
        wait_ui(&view, cx, "copyable PTY screen", |view| {
            view.snapshot.as_ref().is_some_and(|snapshot| {
                snapshot
                    .cells
                    .iter()
        ...
        // R47 §2.3: with the toolbar hidden while Running, copy is carried by
        // the existing ⌘C → `copy_screen` action instead of a toolbar button.
        ...
        assert_eq!(
            ...
            "⌘C copies the visible PTY screen while the toolbar is hidden"
        );
        window
            .update(cx, |view, window, cx| {
                view.replace_text_in_range(None, "/bin/sh ./ui-foreground.sh", window, cx)
            })
            .unwrap();
        cx.simulate_keystrokes(window.into(), "enter");
        wait_ui(&view, cx, "foreground interrupt readiness", |_| {
            root.join("ui-foreground-ready").exists()
        });
        let prompt_before_interrupt =
            prompt_epoch(&root).expect("shell already displayed a prompt");
        cx.simulate_keystrokes(window.into(), "ctrl-c");
        wait_ui(&view, cx, "shell recovered after Ctrl-C", |view| {
            root.join("ui-interrupted").exists()
                && prompt_after(view, &root, prompt_before_interrupt)
        });
        assert_eq!(
            std::fs::read_to_string(root.join("ui-interrupted")).unwrap(),
            "interrupted",
            "Ctrl-C reached the foreground process's installed signal handler"
        );
        assert!(
            !root.join("ui-uninterrupted").exists(),
            "the foreground command was interrupted, not allowed to complete"
        );
        window
            .update(cx, |view, window, cx| {
            ...
            .unwrap();
        cx.simulate_keystrokes(window.into(), "enter");
        wait_ui(&view, cx, "shell exit 9", |view| {
            view.snapshot
                .as_ref()
                .is_some_and(|s| s.status == TerminalStatus::Exited(9))
        });
        // R47 §2.3: abnormal states render the status toolbar again with the
        // frozen R22 geometry (these assertions moved here from the Running
        // phase, where the row no longer exists).
        {
            let mut visual = VisualTestContext::from_window(window.into(), cx);
            let toolbar = visual
                .debug_bounds("terminal-toolbar")
                .expect("exited terminal keeps its status toolbar");
            let status = visual
        ...
        let prompt_before_restart = prompt_epoch(&root).expect("previous shell prompt epoch");
        let mut visual = VisualTestContext::from_window(window.into(), cx);
        visual.simulate_click(restart.center(), Modifiers::default());
        visual.run_until_parked();
        wait_ui(&view, cx, "restarted shell prompt", |view| {
            view.snapshot
                .as_ref()
                .is_some_and(|s| s.status == TerminalStatus::Running)
                && prompt_after(view, &root, prompt_before_restart)
        });
            ...
            .is_some_and(|snapshot| matches!(snapshot.status, TerminalStatus::Exited(_)))
        {
            assert!(
                Instant::now() < until,
                "terminal close/reap timeout: {:?}",
                session.snapshot(None).map(|s| (s.status, s.generation))
            );
            std::thread::sleep(Duration::from_millis(15));
        }
    }
    ...
    async fn r47_terminal_running_hides_the_status_toolbar(cx: &mut TestAppContext) {
        let (_, window) = mount_unit_view(TerminalStatus::Running, cx);
        let mut visual = VisualTestContext::from_window(window.into(), cx);
        assert!(
