...
fn mounted_provider_credential_states_fixture(
    cx: &mut TestAppContext,
) -> (
    tempfile::TempDir,
    Entity<SettingsView>,
...
#[gpui_kit::test]
async fn issue81_provider_settings_remove_pi_import_and_keep_credential_states(
    cx: &mut TestAppContext,
) {
    let (root, view, window) = mounted_provider_credential_states_fixture(cx);
    let mut visual = VisualTestContext::from_window(window.into(), cx);
    assert!(visual.debug_bounds("provider-key-stored").is_some());
    assert!(visual.debug_bounds("provider-key-missing").is_none());
    assert!(visual.debug_bounds("provider-import-pi").is_none());
    assert!(visual.debug_bounds("provider-add").is_some());
    assert!(visual.debug_bounds("provider-edit").is_some());
    assert!(visual.debug_bounds("provider-enabled").is_some());
    ...
    assert_eq!(
        keystore::get_key(root.path(), "stored").unwrap(),
        "issue82-test-secret"
    );
    click(cx, window, "provider-enabled");
    assert!(
        !config::read_from(&root.path().join("config.toml"))
            .unwrap()
            .providers[0]
            .enabled
    );
