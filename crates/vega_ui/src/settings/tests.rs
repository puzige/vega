...
async fn issue73_mcp_disable_remains_clickable_during_a_busy_connection(cx: &mut TestAppContext) {
    ...
    let created = service
        ...
        .expect("saved server");
    let store = vega_store::Store::open(owned.path().join("vega.db")).unwrap();
    vega_store::mcp_servers::set_enabled(store.conn(), &created.id, created.config_revision, true)
        .unwrap();
    cx.update(|cx| {
        cx.set_global(vega_theme::Theme::light());
        cx.set_global(SettingsOpen(true));
...
impl OwnedOAuthMetadataFixture {
        ...
        let endpoint = vega_mcp::mock::Endpoint::new("/mcp", |origin| {
            ...
            Arc::new(move |request| {
                ...
                accepted_paths_worker
                    .lock()
                    .unwrap()
                    .push(format!("{path} | {status}"));
                let response = format!(
                    "HTTP/1.1 {status}\r\nContent-Type: application/json\r\nContent-Length: {}\r\n{headers}\r\n{body}",
                    body.len()
                );
                Box::pin(async move { Ok(vega_mcp::mock::raw_response(response)) })
            })
        });
        Self {
    ...
    fn complete_callback(&self, redirect_uri: &str, authorization_url: &str) -> Vec<u8> {
        ...
        let mut stream =
            vega_conversation::McpServerSettingsService::test_oauth_callback_stream(redirect_uri);
        let request = format!(
...
fn assert_slow_test_then_revoke(cx: &mut TestAppContext, disable: bool) {
    ...
    let _fixture = vega_mcp::mock::StdioFixture::new(
        ...
        Arc::new(move |server, stream| {
            let first = starts.fetch_add(1, Ordering::SeqCst) == 0;
            Box::pin(async move {
                if first {
                    return;
                }
                std::fs::write(&server.args[1], "started").unwrap();
                vega_mcp::mock::serve_json(stream, |_| std::future::pending()).await;
            })
        }),
    );
    let service = vega_conversation::McpServerSettingsService::new(
