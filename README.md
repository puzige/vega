...
| Workflow | 触发 | 检查 |
|---|---|---|
| [`.github/workflows/pr-check.yml`](.github/workflows/pr-check.yml) | `pull_request`（base `master`） | 并行 quality（fmt / clippy / doc-tests）+ 4 个 nextest hash 分片；汇总为 `check (fmt, clippy, test)`，全部成功才放行 |
